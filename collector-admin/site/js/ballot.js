...
function loadPageData() {
  ...
  $.getJSON('data/status.json', function(state) {
      ...
      if ((state.collector_state == 'NOT INSTALLED') ||
        ...
        $('#common-warning-msg .panel-body p').text('Kogumisteenus pole seadistatud. Urni allalaadimine pole võimaik.');
        $('#panel-download-form').hide();
      } else if (state.election.phase == 'PREPARING') {
        $('#common-warning-msg').show('slow');
        $('#common-warning-msg .panel-body p').text('Hääletamine on ettevalmistamise faasis. Urni allalaadimine pole võimaik.');
        $('#panel-download-form').hide();
      } else if (state.election.phase != 'FINISHED') {
        $('#common-warning-msg').show('slow');
        $('#common-warning-msg .panel-body p').text('Hääletusperioodil on võimalik väljastada vaid e-urni varukoopiat.');
  ...
  $.getJSON('cgi/ballot-box-state', function(state) {
      ...
      $(state.data).each(function() {
        ...
        var message = this.filename.search('consolidated') === -1
          ? 'Eksporditud e-urn'
          : 'Eksporditud ja konsolideeritud e-urn';
        if (this.state == 'ready') {
    ...
    .fail(function(jqXHR, textStatus) {
      ...
      line
        ...
        .first()
        .text(
          'Serveri viga allalaadimiseks ettevalmistatud urnide olekuandmete laadimisel');
  ...
  $.ajax({
    ...
    success: function(url) {
      ...
      $('#panel-download-form .panel-body')
        .text('Server alustas urni ettevalmistamist allalaadimiseks');
    },
