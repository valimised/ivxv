...
käivitada mõnes ``ivxv-storage`` teenuse masinas ja kasutajakonto
 ...
 * RAFT index - etcd kirjutamisoperatsioonide arv (kaasaarvatud
...
olemasolev juht mingil põhjusel ei vasta piisavalt kiiresti klastri liikmete
päringutele.

Käsurea seletus:

 * ``env ETCDCTL_API=3``: kasutame etcd API versiooni 3 (Ubuntu Bionicus on
   ``etcdctl`` API vaikeversioon veel 2).
 * ``--cacert``: usaldame ainult servereid, kelle sertifikaat on antud selle CA
   poolt.
...
Väljundit saab küsida ka JSON-kujul (``-w json``), kui see Zabbixile paremini
söödav on:
